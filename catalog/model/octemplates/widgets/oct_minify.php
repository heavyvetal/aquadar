...
class ModelOCTemplatesWidgetsOctMinify extends Model {
	public function octMinifyCss($data) {
		$oct_feelmart_data = isset($this->config->get('theme_oct_feelmart_data')['minify']) ? 1 : 0;
		$oct_styles = $this->document->getOctStyles();

		if ($oct_feelmart_data) {
			$oct_feelmart_version = "?v=" . $this->config->get('theme_oct_feelmart_version');
			...
			if (file_exists($dir_minify . 'cache/' . $oct_name)) {
				...
				$oct_styles[0] = [
					'href' => 'min/cache/' . $oct_name . $oct_feelmart_version,
					'rel' => 'stylesheet',
					'media' => 'screen'
				];
	...
	public function octMinifyJs() {
		...
		if ($oct_feelmart_data) {
			...
			if (file_exists($dir_minify . 'cache/' . $oct_name)) {
				...
				$oct_scripts[0] = 'min/cache/' . $oct_name . $oct_feelmart_version;
